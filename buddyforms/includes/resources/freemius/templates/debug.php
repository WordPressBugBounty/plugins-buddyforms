 ...
 $sites_map = $VARS[ $module_type . '_sites' ]; $all_plans = false; ?>
    <?php if ( is_array( $sites_map ) && count( $sites_map ) > 0 ) : ?>
        <h2>
            ...
            <?php echo esc_html( sprintf( fs_text_inline( '%s Installs', 'module-installs' ), ( WP_FS__MODULE_TYPE_PLUGIN === $module_type ? fs_text_inline( 'Plugin', 'plugin' ) : fs_text_inline( 'Theme', 'theme' ) ) ) ) ?> / <?php fs_esc_html_echo_x_inline( 'Sites', 'like websites', 'sites' ) ?>
        </h2>
...
<?php endforeach ?>
<?php
 $page_params = array( 'is_fs_debug_page' => true ); fs_require_template( 'debug/scheduled-crons.php', $page_params ); ?>
